...
void DataFile::printDisplayList() {
    ...
    if(_displayList.empty()) { 
        ...
        while (displayListIter != _displayList.end()) {
            displayString = (**displayListIter).getString();
            indexCount = (*(*displayListIter)).getIndex();
            sprintf_s(buffer, MESSAGE_DISPLAY_CONTENTS.c_str(),indexCount , displayString.c_str());
            std::cout << buffer << std::endl;
            displayListIter++;
            indexCount++;
        }
